...
/*  standardize.php
    Last update: 6/5/09
    Last updated by: Woon (woonkhang@gmail.com)
    Fixed: 1) Problem with removing from part of a valid call no (eg, BF 1990 where 1990 is removed). 
              Fixed this by adding another 5th restriction to date assumption.
           2) Problem with fill_z_behind(). Before it's fixed, ending call no such as BG 1900 will be BG 19, 
              because the 0's at the back are filled with Z's. It's fixed now to ensure that 0's at the end will not be replaced with Z's.
    Fixed: Problem with removing year at back of call no, sometimes it removes part of the call no, which is mistaken as year. (1/15/08)
    Changes: Redesigned standardize function. (6/25/07)
	Standardize function is required so that the call no can be searched properly and easily.
	...
	b) code_extract function
		1) extract each section of codes into an array (ie, NA237.45 C4 M6 has total 7 sections, which are NA, 237, 45, C, 4, M, 6)
    ...
    Special notes: 
    This script made the following assumptions when removing a date from the call no (which is not useful for searching).
...
/*  Input:  A regular library call number, usually for search purposes.
    Output: A call number with nine digit segments separated by dots. ie, AL123.4 C23 M4 will be standardized as AL0000123.400000000.C23000000.M40000000 
...
/* Fill Z behind when inserting a new std_end entry into db (to solve the continuity problem, so that it connects to the start of the next range)  */
...
/* Make call no. all uppercase, remove date, strips white spaces and dots, make decimal point to '*' */
function process($callnum)
{
	/* Make all alphabets uppercase */
	$callnum = strtoupper($callnum);

	/* Delete the year of the call number if exist (the year must have a space in front, starts with digit 1 or 2, and may or may not have a single character after the year. It must also consist of more than 2 parts. */
...
/* Count the number of parts in a call number. This is used only in the process function
   to ensure that it doesn't delete part of the call number as date, eg: BF 1990. */
...
/* Extract the code in sections and store in an array
     The callnum provided should contain only either alphabets, digits, and a special character (*), without any whitespaces
     ie, NA1539*73M23P35
	 will be extracted as
	$result[0] = "NA", $result[1] = "1539", $result[2] = "73", $result[3] = "M", $result[4] = "23", $result[5] = "P", $result[6] = "35"

	NA1539M23P35
	$result[0] = "NA", $result[1] = "1539", $result[2] = "", $result[3] = "M", $result[4] = "23", $result[5] = "P", $result[6] = "35"
 */
...
function code_extract($callnum)
{
	...
	//Replace "OVERSIZE" in call number with "OVSZ" in order to work with following logic
	...
	for ($i = 0; $i < strlen($callnum_s); $i++)
	{
		...
		if ($is_letter)	// if current char is a letter
		{
			if (is_digit($curr_char))	// the current char change from letter to digit, so start extracting next section
			{
				$is_letter = false;
				$code_i++;
				$result[$code_i] = "";
		...
		{
			if ($curr_char == "*")	// check for the special *, which is the decimal point, if it is, advance to next section and ignore the *
			{
				$code_i++;
				$result[$code_i] = "";
				continue;
			}
			else if (!is_digit($curr_char))	// the current char change from digit to letter, so start extracting next section
			{
				if ($code_i == 1)	// the call number does not have decimal point, so set decimal as empty string, and advance to next section
				{
					$code_i++;
					$result[2] = "";
				}
